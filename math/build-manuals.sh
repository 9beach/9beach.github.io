...
# Poor dirty logic
for i in jech-set-theory-solutions/*html munkres-topology-solutions/*html hungerford-algebra-solutions/*html; do echo '---\ntitle:' `grep '<title>' $i | sed -e 's:.*<title>:":' -e 's:</title>:":'` > .math.tmp; echo 'layout: post\ncomments: true\n---\n\n{% raw %}' >> .math.tmp; cat $i | sed -e 's:README.html:../index.html:' -e 's:<style type="text/css">:&\
